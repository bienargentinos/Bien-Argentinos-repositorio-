...
function normalizarUrlAudio(pathOrUrl) {
  ...
  if (u.indexOf('/almacenamiento/') !== -1) {
    ...
    var filename = u;
    while (filename.charAt(0) === '/' || filename.charCodeAt(0) === 92) {
...
function parseAudiosDetallados(datos) {
  ...
  var audioUrlRegex = new RegExp('(/(root/marcos|archivos|almacenamiento)[^"\\s>\\]]+\\.(ogg|mp3|wav|m4a|aac|opus|webm)|https?:\\/\\/[^"\\s>\\]]+\\.(ogg|mp3|wav|m4a|aac|opus|webm)|https?:\\/\\/[^"\\s>\\]]*audio[^"\\s>\\]]*)', 'gi');
...
function procesarLineaMultimediaChat(strText, isMarcos) {
...
function renderizarBloqueChat(rawChat, tipoBloque, datos) {
  ...
  if (!chatLines.length && tipoBloque === 'vecino' && datos.historial_chat) {
    ...
    chatLines = chatLines.filter(function(line) {
      ...
      return !/proveedor|t.cnico|plomero|electricista|gasista|instalador|service|marcos.*proveedor|marcos.*t.cnico/i.test(str);
    });
    ...
    chatLines = allGen.filter(function(line) {
      var str = typeof line === 'object' ? ((line.emisor || '') + ': ' + (line.texto || '')) : String(line);
      return /proveedor|t.cnico|plomero|electricista|gasista|instalador|service|julio|dario/i.test(str) ||
             /marcos.*(proveedor|t.cnico|julio|dario|tecnico)/i.test(str) ||
             (typeof line === 'object' && (line.rol === 'proveedor' || line.tipo === 'proveedor'));
    });
  }
  ...
  if (chatLines.length > 0) {
    var bubbles = chatLines.map(function(line) {
      ...
      var isProveedor = /^(Proveedor|Técnico|Plomero|Electricista|Gasista|Instalador)/i.test(str) || /marcos.*(proveedor|t.cnico|julio|dario)/i.test(str) || (typeof line === 'object' && (line.rol === 'proveedor' || line.tipo === 'proveedor'));
      ...
      var isMarcos = /^(Marcos|IA|Sistema|🤖)/i.test(str) || (!isVecino && !isProveedor && !isEncargado && !isFamiliar);
      ...
      if (isFamiliar) {
        ...
        if (str.indexOf('Marcos (a Proveedor') !== -1 || str.indexOf('Marcos ➔ Proveedor') !== -1) {
          senderLabel = 'Marcos ➔ Proveedor';
          align = 'margin-right:auto;background:#FFFBEB;color:#78350F;border:1px solid #FDE68A;border-bottom-left-radius:2px;';
        } else {
      ...
      var mediaRes = procesarLineaMultimediaChat(cleanText, isMarcos);
...
function parseStaffClient(namesStr, telsStr) {
  ...
  for (var i = 0; i < rawNames.length; i++) {
    ...
    if (isScheduleFragment && res.length > 0) {
      var cleanHor = str.replace(new RegExp(lb + '[^' + rb + ']*' + rb, 'g'), '').replace(new RegExp(rb, 'g'), '').replace(new RegExp('^[^a-z0-9]+', 'i'), '').trim();
      if (cleanHor) {
    ...
    var matchMeta = str.match(new RegExp(lb + '(activo|licencia|vacaciones)?\\s*\\|?\\s*([^' + rb + ']*)' + rb, 'i'));
    if (matchMeta) {
      if (matchMeta[1]) estado = matchMeta[1].toLowerCase();
      if (matchMeta[2]) horario = matchMeta[2].trim();
      str = str.replace(new RegExp(lb + '[^' + rb + ']*' + rb, 'g'), '').trim();
    }
