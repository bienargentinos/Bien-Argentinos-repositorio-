...
const SESSION_SECRET = process.env.DASHBOARD_SECRET || 'marcos-secret-cambiar-en-produccion-2024';
const SHEET_ID = process.env.GOOGLE_SHEET_ID;
const CREDENTIALS_FILE = process.env.GOOGLE_CREDENTIALS_FILE || 'gen-lang-client-0735429936-bba6999e5e60.json';
...
async function getSheetsClient() {
  ...
  const keyFile = path.isAbsolute(CREDENTIALS_FILE) ? CREDENTIALS_FILE : path.join(__dirname, CREDENTIALS_FILE);
...
async function readTab(tabName) {
  ...
  let res;
  try {
    res = await sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: `${tabName}!A:Z` });
  ...
  const rows = values.slice(1).map((row, idx) => {
    ...
    headers.forEach((h, i) => { if (!h) return; obj[h] = row[i] !== undefined ? row[i] : ''; });
    return obj;
  });
...
function normalizeKey(k) {
  return String(k || '').trim().toLowerCase().normalize('NFD').replace(/\p{Diacritic}/gu, '').replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '');
}
...
function mapEdificio(r) {
  return {
    ...
    telefonos: pick(r, ['telefonos', 'contactos', 'numeros']),
...
function requireAuth(req, res, next) {
  ...
  if (req.headers.accept && req.headers.accept.includes('application/json')) return res.status(401).json({ error: 'No autenticado' });
...
async function guardarEdificio(btn, row){
  var tr=btn.closest('tr');var data={row:row};
  tr.querySelectorAll('input[data-field]').forEach(function(inp){data[inp.getAttribute('data-field')]=inp.value;});
  btn.disabled=true;var old=btn.textContent;btn.textContent='...';
  try{
    var r=await fetch('/admin/api/edificio',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});
    var j=await r.json();
    if(!r.ok||j.error)throw new Error(j.error||'Error');
...
function aplicarFiltros(){
  var fe=document.getElementById('f-edificio');var fu=document.getElementById('f-urgencia');var ft=document.getElementById('f-texto');
  var edi=fe?fe.value:'';var urg=fu?fu.value:'';var txt=ft?ft.value.toLowerCase():'';
  var vis=0;
  document.querySelectorAll('.event[data-edificio]').forEach(function(ev){
    ...
    var show=okE&&okU&&okT;ev.style.display=show?'':'none';if(show)vis++;
  });
  var none=document.getElementById('no-results');if(none)none.style.display=vis===0?'block':'none';
}
...
<header class="topbar">
  <div class="inner">
    ...
    <nav class="nav">
      ${nav.map(([href, label, id]) => `<a href="${href}" class="${active === id ? 'active' : ''}">${label}</a>`).join('')}
    </nav>
...
router.get('/login', (req, res) => {
  ...
  const err = req.query.error ? `<div class="err">Usuario o contrasena incorrectos.</div>` : '';
...
<div class="login-wrap">
  <form class="login-card" method="POST" action="/admin/login">
    ...
    ${err}
    <div class="field"><label>Usuario</label><input name="user" autocomplete="username" autofocus required></div>
    <div class="field"><label>Contrasena</label><input name="pass" type="password" autocomplete="current-password" required></div>
...
router.get('/', async (req, res) => {
  try {
    ...
    const ult = [...eventos].sort((a, b) => (parseFecha(b.fecha) || 0) - (parseFecha(a.fecha) || 0)).slice(0, 6);
    ...
    const ultHtml = ult.length ? `<div class="feed">${ult.map(renderEventoMini).join('')}</div>` : `<div class="empty">Todavia no hay eventos registrados.</div>`;
    res.send(page('resumen', 'Resumen', `<h1>${saludo}, tomate unos mates &#x1F9C9;</h1><p style="color:var(--muted);margin-top:-8px">Esto es lo que paso mientras dormias.</p>${cards}<h2>Ultimos movimientos</h2>${ultHtml}<p style="margin-top:18px"><a class="btn ghost sm" href="/admin/eventos">Ver todos los eventos →</a></p>`));
  } catch (e) { res.status(500).send(page('resumen', 'Resumen', errorBox(e))); }
});
...
function tipoLabel(t) {
  return { texto: '&#x1F4AC; WhatsApp', audio: '&#x1F399;&#xFE0F; Audio', llamada: '&#x1F4DE; Llamada', imagen: '&#x1F5BC;&#xFE0F; Imagen' }[t] || '&#x1F4AC; Mensaje';
}

function renderEventoMini(e) {
  return `<div class="event">
    <div class="head">
      <div><div class="who">${esc(e.edificio)} · ${esc(e.vecino)}</div><div class="meta">${esc(fechaCorta(parseFecha(e.fecha)) || e.fecha)}</div></div>
      <div class="badges"><span class="badge tipo">${tipoLabel(e.tipo)}</span><span class="badge ${e.urgencia}">${e.urgencia}</span></div>
    </div>
...
function renderEventoFull(e) {
  const transcript = e.transcripcion ? `<div class="transcript"><span class="lbl">Transcripcion del audio</span>${esc(e.transcripcion)}</div>` : '';
  const resumen = e.resumen ? `<div class="meta" style="margin-top:8px"><b>Marcos:</b> ${esc(e.resumen)}</div>` : '';
  ...
  return `<div class="event" data-edificio="${esc(e.edificio)}" data-urgencia="${esc(e.urgencia)}">
    <div class="head">
      <div><div class="who">${esc(e.edificio)} · ${esc(e.vecino)}</div><div class="meta">${esc(fechaCorta(parseFecha(e.fecha)) || e.fecha)}${e.telefono ? ' · ' + esc(e.telefono) : ''}</div></div>
      <div class="badges"><span class="badge tipo">${tipoLabel(e.tipo)}</span><span class="badge ${e.urgencia}">${e.urgencia}</span></div>
    </div>
    ${e.mensaje ? `<div class="body">${esc(e.mensaje)}</div>` : ''}
    ${transcript}${resumen}
    <div class="fb">${existing}<div class="row"><textarea data-fb placeholder="Dejale una nota a Marcos para que aprenda de este caso...">${esc(e.feedback || '')}</textarea><button class="btn sm" onclick="guardarFeedback(this, ${e._row})">Guardar nota</button></div></div>
  </div>`;
}
...
router.get('/eventos', async (req, res) => {
  try {
    ...
    const feed = eventos.length
      ? `<div class="feed">${eventos.map(renderEventoFull).join('')}</div><div id="no-results" class="empty" style="display:none">No hay eventos que coincidan con el filtro.</div>`
      : `<div class="empty">Todavia no hay eventos registrados.</div>`;
    res.send(page('eventos', 'Eventos', `<h1>Feed de eventos</h1><div class="filters"><label class="flt">Edificio<select id="f-edificio" onchange="aplicarFiltros()"><option value="">Todos</option>${opcionesEdi}</select></label><label class="flt">Urgencia<select id="f-urgencia" onchange="aplicarFiltros()"><option value="">Todas</option><option value="alta">Alta</option><option value="media">Media</option><option value="baja">Baja</option></select></label><label class="flt">Buscar<input id="f-texto" placeholder="vecino, texto..." oninput="aplicarFiltros()"></label></div>${feed}`));
  } catch (e) { res.status(500).send(page('eventos', 'Eventos', errorBox(e))); }
});
...
router.get('/archivos', async (req, res) => {
  try {
    ...
    const body = archivos.length ? `<div class="grid-files">${archivos.map(renderArchivo).join('')}</div>` : `<div class="empty">No hay facturas ni fotos cargadas todavia.</div>`;
    res.send(page('archivos', 'Facturas/Fotos', `<h1>Facturas y fotos</h1><p style="color:var(--muted);margin-top:-8px">Archivos que enviaron vecinos o proveedores.</p>${body}`));
  } catch (e) { res.status(500).send(page('archivos', 'Facturas/Fotos', errorBox(e))); }
});

function esImagen(url) { return /\.(png|jpe?g|webp|gif)(\?|$)/i.test(url || ''); }
...
function renderArchivo(a) {
  ...
  return `<div class="file"><div class="thumb">${thumb}</div><div class="info"><div class="t">${esc(a.descripcion || a.tipo || 'Archivo')}</div><div class="m">${esc(a.edificio)} · ${esc(a.enviado_por)}</div><div class="m">${esc(fechaCorta(parseFecha(a.fecha)) || a.fecha)}</div>${a.url ? `<div style="margin-top:8px"><a class="btn ghost sm" href="${esc(a.url)}" target="_blank" rel="noopener">Abrir →</a></div>` : ''}</div></div>`;
}
...
router.get('/edificios', async (req, res) => {
  try {
    ...
    const body = edificios.length
      ? `<div class="tablewrap"><table><thead><tr><th>Edificio</th><th>Encargado</th><th>Tel. enc.</th><th>Administrador</th><th>Propietarios</th><th>Telefonos</th><th></th></tr></thead><tbody>${filas}</tbody></table></div>`
      : `<div class="empty">No hay edificios cargados.</div>`;
    res.send(page('edificios', 'Edificios', `<h1>Datos de edificios</h1><p style="color:var(--muted);margin-top:-8px">Edita encargados, propietarios y telefonos. Los cambios se guardan en Google Sheets.</p>${body}`));
  } catch (e) { res.status(500).send(page('edificios', 'Edificios', errorBox(e))); }
});

function errorBox(e) {
  return `<div class="empty"><p>Ups, no pude leer los datos de Google Sheets.</p><p style="font-size:13px;color:var(--bad)">${esc(e && e.message ? e.message : e)}</p><p style="font-size:13px">Revisa GOOGLE_SHEET_ID, las credenciales y los nombres de las pestanias.</p></div>`;
}
...
async function writeCell(tabName, col, row, value) {
  const sheets = await getSheetsClient();
  await sheets.spreadsheets.values.update({ spreadsheetId: SHEET_ID, range: `${tabName}!${col}${row}`, valueInputOption: 'USER_ENTERED', requestBody: { values: [[value]] } });
}

async function findOrPlanColumn(tabName, candidateKeys) {
  const { rawHeaders, headers } = await readTab(tabName);
  for (let i = 0; i < headers.length; i++) {
    if (candidateKeys.includes(headers[i])) return { col: columnLetter(i + 1), index: i, rawHeaders, headers };
  }
  return { col: columnLetter((rawHeaders ? rawHeaders.length : 0) + 1), index: rawHeaders ? rawHeaders.length : 0, create: candidateKeys[0], rawHeaders: rawHeaders || [], headers: headers || [] };
}

async function ensureHeader(tabName, col, name) {
...
router.post('/api/feedback', async (req, res) => {
  try {
    ...
    if (plan.create) await ensureHeader(TAB_EVENTOS, plan.col, 'feedback');
...
router.post('/api/edificio', async (req, res) => {
  try {
    ...
    for (const field of Object.keys(EDIFICIO_FIELDS)) {
      ...
      else { col = columnLetter(workingHeaders.length + 1); await ensureHeader(TAB_EDIFICIOS, col, candidates[0]); workingHeaders.push(candidates[0]); }
