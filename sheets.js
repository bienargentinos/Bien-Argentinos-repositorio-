...
async function guardarReporte({ edificio, vecino, depto, problema, urgencia, estado, notas, notas_ia, fechaInicio, tipo = 'whatsapp', telefono = '', audio_url = '', transcripcion = '', historial_chat = '', id_evento = '', tecnico = '', tel_tecnico = '', rubro_tecnico = '' }) {
    try {
        ...
        chatNuevosArr.forEach(m => {
            const strM = typeof m === 'object' ? (JSON.stringify(m) + ' ' + (m.emisor || m.remitente || '') + ' ' + (m.texto || m.mensaje || '')) : String(m || '');
            const rawText = typeof m === 'object' ? String(m.texto || m.mensaje || '') : String(m || '');
            const isProv = /proveedor|t.cnico|instalador|plomero|electricista|gasista|marcos ➔ proveedor|marcos -> proveedor|marcos \(a proveedor\)|pudiste ir|pudiste realizar|pudiste asistir|pudiste pasar|factura|comprobante|\[factura:/i.test(strM);
            const itemToSave = rawText || (typeof m === 'string' ? m : strM);
            if (isProv) {
                if (!chatProveedorLista.includes(itemToSave)) chatProveedorLista.push(itemToSave);
            } else {
                if (!chatVecinoLista.includes(itemToSave)) chatVecinoLista.push(itemToSave);
            }
        });
